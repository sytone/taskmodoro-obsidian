...
function unBold(str: string): string {
    return str.replace(isBoldedRegEx, '$1');
}

const isItalicizedRegEx = /^[*_]{1}(.+)[*_]{1}$/;
function unItalicize(str: string): string {
...
function unHighlight(str: string): string {
    return str.replace(isHighlightedRegEx, '$1');
}

const isStrikedRegEx = /^~{2}(.+)~{2}$/;
function unStrike(str: string): string {
    return str.replace(isStrikedRegEx, '$1');
}

const isQuoted = /^(?:>.+?(?:[\r\n]|$))+$/;
function applyQuote(str: string): string {
    const quoted = str
        .split('\n')
        .map((line) => {
            if (line[0] === '>') {
...
function getOrdinal(line: string): number {
    return parseInt(line.match(/^\s*(\d+)/)[1], 10);
}

function applyOrderedList(str: string): string {
...
function removeOrderedList(str: string): string {
    return str
        .split('\n')
        .map((line) => {
            if (!isOrderedList.test(line)) {
...
function applyBullet(str: string): string {
    const bulleted = str
        .split('\n')
        .map((line) => {
...
function bulletToEmptyCheckbox(str: string): string {
    return str
        .split('\n')
        .map((line) => {
...
function orderedListToEmptyCheckbox(str: string): string {
    return str
        .split('\n')
        .map((line) => {
...
function checkedToEmpty(str: string): string {
    return str
        .split('\n')
        .map((line) => {
...
function orderedCheckedToEmpty(str: string): string {
    return str
        .split('\n')
        .map((line) => {
...
function emptyToChecked(str: string): string {
    return str
        .split('\n')
        .map((line) => {
...
function orderedEmptyToChecked(str: string): string {
    return str
        .split('\n')
        .map((line) => {
...
function removeBullet(str: string): string {
...
export function unpairBrackets(textarea: HTMLTextAreaElement): boolean {
    return unpair(textarea, autoPairBracketsCommands);
}

export function unpairMarkdown(textarea: HTMLTextAreaElement): boolean {
    return unpair(textarea, autoPairMarkdownCommands);
}

function applyTab(str: string, useTab: boolean, tabWidth: number): string {
...
function removeTab(str: string, useTab: boolean, tabWidth: number): string {
    const tab = useTab ? '\\t' : ' '.repeat(tabWidth);
...
export function handleTab(
    ...
    const withInitializedOrdinal = withTab.replace(
        /^(\s*)(\d+)([.)]\s)/,
        (_, space, number, after) => `${space}1${after}`,
    );
...
export function handleNewLine(textarea: HTMLTextAreaElement): boolean {
    const initialState = getStateFromTextarea(textarea);
    ...
    if (isOrderedList.test(before)) {
        ...
        const withEmptyCheckbox = pre.replace(
            /^(\s*\d+[.)]\s+)\[[^\]]\]/,
            '$1[ ]',
        );
        const withIncrementedOrdinal = withEmptyCheckbox.replace(
            /^(\s*)(\d+)/,
            (_, space, number) => `${space}${parseInt(number) + 1}`,
        );
