...
export function getSurroundingLines(text: string, start: number, end?: number): { start: number; end: number; } {
...
export function getSurroundingWord(text: string, start: number, end?: number): { start: number; end: number; } {
  if (!text) throw Error("Argument 'text' should be truthy");
...
export function selectWord({ text, selection }: TextSection): { start: number; end: number; } {
...
export function replaceSelection(textarea: HTMLTextAreaElement, text: string): TextState {
